...
def new(Doc: type[D]) -> D:
	"""
	Custom `new_doc` implementation for llm. Invocation uses class instead of doctype.
...
def get(Doc: type[D], name: str, *, cached: bool = False) -> D:
	"""
	Custom `get_doc` implementation for llm. Invocation uses class instead of doctype.
