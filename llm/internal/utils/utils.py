...
from llm.core.types import SessionStats, SessionToolUseStats
from llm.internal import utils

if TYPE_CHECKING:
	from llm.core.types import Provider, Query, ReasoningEffort, Session, ToolUseContent
...
if TYPE_CHECKING:
	from llm.core.types import Meta, SessionItem, ToolUseUpdate, UserContent
...
def to_content(query: Query) -> list[UserContent]:
	...
	"""
	from llm.core.types import FileContent, ImageContent, TextContent
...
def get_key(provider: Provider) -> tuple[str, str] | tuple[None, None]:
	"""Get API key name and value for provider. Checks envvar and LLM Settings.
	...
	if not password:
		password = get_decrypted_password("LLM Settings", "LLM Settings", key.lower())
