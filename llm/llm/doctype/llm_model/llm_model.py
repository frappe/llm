...
from llm.core.utils import DEFAULT_INSTRUCTION, is_reasoning_effort

if TYPE_CHECKING:
	from llm.core.types import ModelSize, Provider, ReasoningEffort
...
class LLMModel(Document):
	...
	def new(
		...
		doc = cast("LLMModel", frappe.get_doc({"doctype": "LLM Model"}))
	...
	def ask(self, query: str, instruction: str | None = None, reasoning_effort: str | None = None):
		...
		from llm.lib import quick_query
...
def get_reasoning_effort(effort: str | None, llm: LLMModel | None = None) -> ReasoningEffort | None:
	if not effort:
		return None
