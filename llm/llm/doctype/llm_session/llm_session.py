...
import llm.core as core
import llm.internal as internal
from llm.core.types import ContentChunk, Query, Session, ToolSchema
from llm.core.utils import get_last_id, get_sequence
from llm.llm.doctype.llm_session_tool_ct.llm_session_tool_ct import LLMSessionToolCT

if TYPE_CHECKING:
	from llm.core.litellm import InteractReturnTuple
	from llm.core.types import SessionItem

logger = internal.logger("llm.doctype.llm_session", level="DEBUG")
...
class LLMSession(Document):
	...
	representation used by the `llm.core` module.

	LLMSession is not meant to be used directly, use the wrapper
	`llm.lib.Session` instead, this exposes specific convenience functions to
	handle a session.
	"""
	...
	if TYPE_CHECKING:
		...
		from llm.llm.doctype.llm_session_item_ct.llm_session_item_ct import LLMSessionItemCT
		from llm.llm.doctype.llm_session_tool_ct.llm_session_tool_ct import LLMSessionToolCT

		first: DF.Data | None
		instruction: DF.Code | None
		is_active: DF.Check
		items: DF.Table[LLMSessionItemCT]
		llm: DF.Link | None
		reason: DF.SmallText | None
		reasoning_effort: DF.Literal["None", "Low", "Medium", "High"]
		tools: DF.Table[LLMSessionToolCT]
		uid: DF.Data | None
	# end: auto-generated types

	_osi_map: dict[str, LLMSessionItemCT] | None = None
	...
	def new(
		*,
		...
		doc = cast("LLMSession", frappe.get_doc({"doctype": "LLM Session"}))
	...
	def interact(self, query: Query | None = None) -> SessionInteractStream:
		...
		"""
		from llm.llm.doctype.llm_model.llm_model import get_reasoning_effort

		self._set_status(is_active=True)
		interact_generator = core.interact(
		...
		while True:
			try:
				...
				internal.log_error(title="interact error", doc=self)
	...
	def set_tools(self, tools: list[ToolSchema]):
		self.tools.clear()
		for tool in tools:
			self.append("tools", LLMSessionToolCT.new(tool))
	...
	def _set_session(self, session: Session) -> None:
		...
		for item in get_sequence(session):
			...
			if not osi:
				osi = LLMSessionItemCT.from_session_item(item)
			else:
	...
	def get_stats(self):
		...
		return core.get_stats(session)

	def _set_osi_map(self) -> None:
		"""Sets _osi_map (LLMSessionItemCT) if not already set, should be used only in get_session and set_session"""
